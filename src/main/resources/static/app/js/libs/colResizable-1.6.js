...
(function($){ 	
	...
	var init = function( tb, options){
		...
		if(t.closest('.tableResponsive').hasClass('overflowMulti')){
			t.find('thead > tr:eq(0) th').each(function(){
				originalColumnWidths.push($(this).outerWidth(true));
			});
		}
		customArrayTotal = originalColumnWidths.reduce(function add(sum,currValue){
			return sum + currValue;
		},0);
		if(originalColumnWidths.length > 0 && customArrayTotal > t.closest('.tableResponsive').width()){
			h.append("<style type='text/css'>  .JColResizer{table-layout:fixed;} .JColResizer > tbody > tr > td, .JColResizer > tbody > tr > th{overflow:hidden}  .JPadding > tbody > tr > td, .JPadding > tbody > tr > th{padding-left:0!important; padding-right:0!important;} .JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-11px; position:absolute; z-index:5; } .JCLRgrip .JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;cursor: col-resize;top:0px} .JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;	} .JCLRFlex.JColWidth{width:max-content !important;} .JCLRgrip.JCLRdisabledGrip .JColResizer{cursor:default; display:none;}</style>");
		}else{
		...
		createGrips(t,originalColumnWidths);		//grips are created
	 ...
	 * @param {jQuery ref} t - table object
	 */
	var createGrips = function(t,originalColumnWidths){/* custom set width */
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			if(originalColumnWidths.length > 0 && customArrayTotal > t.closest('.tableResponsive').width()){
				c.w = originalColumnWidths[i];/* custom set width */
			}
		...
		t.find('.garbage th').css({opacity:0, 'height' : 0,'padding' : 0,'border':'none'});
	};
	...
	*/
	...
	var onGripDrag = function(e){	
		...
		if(t.opt.liveDrag){ 			//if liveDrag is enabled
			...
			if (cb) { e.currentTarget = t[0]; cb(e,x + PX,drag); }		//if any, it is fired
		}
